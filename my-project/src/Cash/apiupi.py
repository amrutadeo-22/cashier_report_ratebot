...
@app.route('/get_entry_upi', methods=['GET'])
def get_data():
    con = get_db_connection()
    cur = con.cursor()
    cur.execute("SELECT * FROM upi")
    data = cur.fetchall()
    cur.close()
    con.close()
    return jsonify(data)

@app.route('/insert_entry_upi', methods=['POST'])
def insert_entry():
    data = request.json
    Sr = data.get("Sr")
    ...
    cur = con.cursor()
    cur.execute("""
        INSERT INTO card (Sr, ProviderOrReceiver, Amount, comment)
